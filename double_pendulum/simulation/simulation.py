...
class Simulator:
    ...
    def run_on_hardware(self, tf, dt, controller=None, actuated_joint=None):
        ...
        shoulder_motor_id = 403 #171
        elbow_motor_id = 379 #172
    ...
    def run_experiment(self, tf, dt, controller=None, experiment_type=None, actuated_joint=None, motors = None):
        ...
        kp=0.0
        kd=0.0
        if experiment_type=="DoublePendulum":
            client.set_impedance_controller_params([kp, kp], [kd, kd])
        else:
            client.set_impedance_controller_params([kp], [kd])
        input(
            "Press bring the pendulum to the starting configuration and press enter to continue..."
        )
        n = int(tf / dt)
        ...
        while i < n:
            ...
            measured_position=client.get_position()
            measured_velocity=client.get_velocity()
            measured_torque=client.get_torque()
            ...
            if (
                ...
                steps = int(1 / dt)
                for _ in range(steps):
                    client.set_impedance_control(kp=np.array([0.0, 0.0]), kd=np.array([0.1, 0.1]))
                    client.set_torque(np.array([0.0, 0.0]))
                    time.sleep(dt)
            ...
            if controller is not None:
                ...
                tau=list(tau)
                if actuated_joint == "elbow":
                    client.set_torque([0.0, tau[1]])
                    tau[0] = 0.0
                elif actuated_joint == "shoulder":
                    client.set_torque([tau[0], 0.0])
                    tau[1] = 0.0
                else:
                    client.set_torque(tau)
            else:
                tau = [0.0, 0.0]
                client.set_torque(tau)
