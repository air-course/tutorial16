...
def frameAcceleration(
    ...
    ref_frame=cpin.ReferenceFrame.LOCAL_WORLD_ALIGNED,
):
    if update_kinematics:
        cpin.forwardKinematics(cmodel, cdata, cq, cv, ca)
    a_local = cpin.getFrameClassicalAcceleration(cmodel, cdata, index, ref_frame)
...
def framePlacement(
    cmodel, cdata, cq, cv, frameIndex, referenceFrame=cpin.ReferenceFrame.WORLD
):
...
def frameVelocity(
    cmodel, cdata, cq, cv, foot_frame_Id, reference_frame=cpin.ReferenceFrame.WORLD
):
    updateFrames(cmodel, cdata, cq, cv)
...
def framePlacementFunctions(
    cmodel, cdata, cq, cv, frameIndex, reference_frame=cpin.ReferenceFrame.WORLD
):
    """
    ...
    rotFunction = casadi.Function(
        f"R_{frameIndex}", [x], [symbolicRep.rotation], ["x"], [f"R_{frameIndex}"]
    )
    posFunction = casadi.Function(
        f"p_{frameIndex}", [x], [symbolicRep.translation], ["x"], [f"p_{frameIndex}"]
    )
    return rotFunction, posFunction


def frameVelocityFunctions(
    cmodel, cdata, cq, cv, frameIndex, reference_frame=cpin.ReferenceFrame.WORLD
):
    """
    Given a frame will output the functions for obtaining the position and rotation from the state
    ...
    x = casadi.vertcat(cq, cv)
    linear = casadi.Function(
        f"R_{frameIndex}", [x], [symbolicRep.linear], ["x"], [f"R_{frameIndex}"]
    )
    angular = casadi.Function(
        f"p_{frameIndex}", [x], [symbolicRep.angular], ["x"], [f"p_{frameIndex}"]
    )
    return linear, angular


def framePlacementFunctionsRedundant(
    cmodel, cdata, cq, cv, nu, frameIndex, reference_frame=cpin.ReferenceFrame.WORLD
):
    """
    Given a frame will output the functions for obtaining the position and rotation from the state
    """
...
def frameVelocityFunctionsRedundant(
    cmodel, cdata, cq, cv, nu, frameIndex, reference_frame=cpin.ReferenceFrame.WORLD
):
    """
    Given a frame will output the functions for obtaining the position and rotation from the state
...
def mechanicalEnergyCostFunction(cmodel, cdata, cq, cv):
    ...
    x = casadi.vertcat(cq, cv)
    mechEnergyFunction = casadi.Function(
        "Mech_Energy", [x], [mechEnergy], ["x"], ["state"]
    )
